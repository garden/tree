...
var defaultRoute = [wsUnit, ajaxUnit, eventSourceUnit,
...
function wsUnit (server) {
  ...
  return function wsLayer (ask, next) {
    // This doesn't actually get run, since ws overrides it at the root.
    if (ask.path.slice(1, 12) !== '$websocket:') return next();
  };
}
...
function staticUnit (server) {
  ...
  return function staticLayer (ask, next) {
    ...
    fs.stat(realpath, function(err, stats) {
      ...
      // Cache management.
      if (+(new Date(ask.req.headers['if-modified-since'])) >= +stats.mtime) {
        ask.res.statusCode = 304;
        ask.res.end();  // not modified.
        return;
      }
      ask.res.setHeader('Last-Modified', stats.mtime.toGMTString());
