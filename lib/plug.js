...
exports.plug = function (query, path, cb) { 
  ...
  fs.file (path[1], function (err, file) {
    ...
    if (err) {
      ...
      cb(err, '/util/404/404.html', {error: err.message});
      return;
    }
    ...
    if (data.lookup('template') === 'no') {
      console.error('sending raw file');
      cb(null, path[1], {});
      return;
    }
    ...
    if (!delayed) {
      //if (!plug) plug = '/util/pencil/pencil.html'; // FIXME no plug found, send raw
