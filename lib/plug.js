...
exports.plug = function (query, path, cb) { 
  ...
  fs.file (path[1], function (err, file) {
    ...
    function end(err, plug, data) {
      if (!plug || plug === 'none') {
        //console.log('ENDING1',err,path[1],JSON.stringify(data||{}));
        cb(err, path[1], data || {});
      } else {
        //console.log('ENDING2',err,plug,JSON.stringify(data||{}));
        cb(err, plug, data || {});
      }
    }
    ...
    if (err) {
      ...
      end(err, '/util/404/404.html', {error: err.message});
      return;
    }
    ...
    if (file.isOfType('dir')) {
      ...
      plug = plug || '/util/gateway/gateway.html';
      ...
      file.files (function (err, files) {
        ...
        for (var i = 0; i < files.length; i++) {
          var filepath = files[i].path + (files[i].isOfType('dir') ? '/' : '');
      ...
      plug = plug || '/util/pencil/pencil.html';
