...
// fleaux = fleau extended (with tree-specific parsers, etc.)
exports.main = function (server, fleaux) { camp = server; fleau = fleaux };
...
exports.resolve = function (query, path, endres, ask) {
  ...
  fs.file (path[1], function (err, file) {
    ...
    // treader: template reader to use.
      ...
      if (!plug || plug === 'none') {
        ...
        fs.file(plug, function(err, file) {
          if (err) sendraw();
            ...
            console.log('treader', file.meta.template);
            endres(data || {}, {
              template: plug,
              reader: file.meta.template === 'fleau'? fleau: null
            });
          }
        });
      }
