...
function launchactions() {
  ...
  camp.addDiffer ('dispatch', function (query) {
    var dispatch = settledispatch(query.user, query.path);
    if (dispatch !== undefined) {
      // That means we have something in cache to sent instantly.
      console.log('registering dispatch differ false');
      return {differ:false, data:dispatch};
    } else {
      console.log('registering dispatch differ true');
      return {differ:true, data:query.user};
    }
