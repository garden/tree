...
tests.push(function testLookup(end) {
  ...
  tfs.file('$test', function(err, folder) {
    ...
    folder.mkfile('file', function(err) {
  ...
  function testLookup(file) {
    ...
    assert(lookup('foo') != null, 'Lookup of direct metadata succeeds');
    assert(lookup('foo').quux === "baz", 'Lookup of direct metadata');
