...
function metaSave(query, end) {
  ...
  fs.file(query.path, function(err, file) {
    if (err) {
      ...
      // Password check.
      if (file.meta['metakey'] != null) {
        if (typeof query.metakey !== 'string') {
          return end({err:'Give a (string) key `metakey` '
            + 'in the query to modify ' + query.path + '.'});
        }
        scrypt.verifyHash(file.meta['metakey'], query.metakey, function(err) {
          if (!!err) {
            end({err:'File ' + query.path + ' requires the correct meta key. '
                + 'Modify `metakey` in the query to match the stored hash.'});
          } else {
            // Start writing.
