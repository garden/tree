...
function upload(query, end, ask) {
  var form = ask.form;
  if (form.error) end({err: form.error}), return;
  fs.file(query.path, function (err, parent) {
    if (err) end({err: err}), return;
    for (var i = 0, files = []; i < form.openedFiles.length; i++) {
      var file = form.openedFiles[i]; files.push(file);
      parent.import(nodepath.basename(file.path), file.name, function() {
        console.log('uploaded ' + nodepath.join(query.path, file.name));
      });
    }
    end({files: files}); // FIXME async?
  });
}
