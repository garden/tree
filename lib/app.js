    ...
    .then(function(paramKeys) {
      ...
      Promise.all(paramKeys.map(function(field) {
        if (field === 'data') {
          ...
          // FIXME: only look up metadata in the user dir and file metadata.
...
function authorization(path, req, res, cb) {
  fs.meta(path).then(function(meta) {
    if (meta === undefined) { if (res) { pageNotFound(res); } return; }
    if (req.user !== undefined) {
      var permission = meta.acl? meta.acl[req.user.name || "*"]: "x";
    } else {
      var permission = meta.acl? meta.acl["*"]: "x";
    }
    if (permission === "-") { if (res) { denyAccess(res); } }
    else { cb(meta, permission); }
  }).catch(function(err) {
    log.error(err);
    if (res) { unloggedError(err, res); }
  });
};
...
exports.resolve = function resolveApp(req, res) {
  ...
  authorization(path, req, res, function(meta, permission) {
...
exports.websocket = function handleWebsocket(req, socket, head) {
  ...
  authenticate(req, undefined, function() {
    authorization(path, req, undefined, function(metadata, permission) {
      if (op === 'edit' && app === 'text') {
