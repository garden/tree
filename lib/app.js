...
function authorize(path, req, res, cb) {
  fs.meta(path).then(function(meta) {
    ...
    fs.metaFind(["acl", username || "*"], path, {meta: meta})
    .then(function(permission) {
      if (permission === undefined && username !== undefined) {
        return fs.metaFind(["acl", "*"], path, {meta: meta})
      } else { return Promise.resolve(permission); }
    }).then(function(permission) {
      if (permission === "-") { if (res) { denyAccess(res); }
      } else { cb(meta, permission || "x"); }
    }).catch(function(err) { error(err, res); });
  }).catch(function(err) { error(err, res); });
};
