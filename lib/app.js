...
function sendApp(app, path, req, res, meta) {
  ...
  if (app === 'data') {
    ...
    fs.getStream(path, {depth: depth}).then(function(stream) { stream.pipe(res.compressed()); })
      ...
      if (app !== undefined) {
        ...
        new Promise(function(resolve, reject) {
          ...
          if (appPath !== undefined) {
            ...
            fs.getStream(path).then(function(stream) { stream.pipe(res.compressed()); });
          }
        }).catch(function(err) { error(err, res); });
      }
    }).catch(function(err) { error(err, res); });
  } else if (req.method === 'PUT') {
    fs.create(path).then(function() { success(null, res, 201); })
    .catch(function(err) { error(err, res); });
