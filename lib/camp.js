  ...
  http.createServer(function(req,res){
    ...
    try {
      ...
      if (/^\/\$/.test (path)) {
        ...
        var platepaths;
        if ((platepaths = Object.keys (platehandled).filter (function(key) {
            return RegExp(key).test (path);
          })).length > 0) {
          ...
          var pathmatch = path.match (RegExp (platepaths[0]));
          var completion = platehandled[platepaths[0]] (query, pathmatch);
