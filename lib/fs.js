 ...
 */
exports.File = function File (typename, name, getcontent) {
  ...
  this.content = function memoizer(dowithcontent) {
    ...
    if (!this._gotcontent) {
      ...
      getcontent(function(err, content) {
        console.log('FS: We just got file.content.');
        // Give them the data.
      ...
      console.log('FS: We just got file.content [cached].');
 ...
 */
exports.getfile = function getfile (path, callback) {
...
function typefromextention (filename) {
  return camp.Server.mime[nodepath(filename).slice(1)];
};
