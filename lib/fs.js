...
File.prototype.subfiles = function (cb, depth) {
  ...
  // complete.
  var paths = [], processedfiles = 0, totalfiles;

  var whengotsubfiles = function(filename) {
    //console.log('FS:SUBFILES: whengotsubfiles called with filename', filename);
    return function(err, subfiles) {
      // Add all subfiles.
      for (var i = 0; i < subfiles.length; i++) {
        //console.log('FS:SUBFILES: %s + %s -> %s', filename, subfiles[i], nodepath.join(filename, subfiles[i]));
        //if (paths.indexOf(nodepath.join(filename, subfiles[i])) !== -1) {
          //console.log('FS:SUBFILES: adding duplicate %s/%s to %s', filename, subfiles[i], paths);
        //}
        paths.push(nodepath.join(filename, subfiles[i]));
      }
      processedfiles++;
      //console.log('FS:SUBFILES: got subfiles %s from filename %s (%s/%s)', subfiles, filename, processedfiles, totalfiles);
      if (processedfiles === totalfiles) {
        //var util = require('util');
        //console.log('FS:SUBFILES: filename %s gives paths %s', filename, util.inspect(paths));
        cb(undefined, paths);
      }
    };
  };
  ...
  this.content(function(err, content) {
    ...
    for (var filename in content) {
      ...
      if (file.isoftype('dir') && depth > 0) {
        ...
        whengotsubfiles(this)(undefined, [filename]);
      }
    }
  });
};
