...
var nodefs = require ('fs'),
    ...
    util = require('util'),
    EditorOTServer = require('ot').EditorSocketIOServer,
    async = require('async');
...
File.prototype.mkdir = function (name, cb) {
  if (this.isOfType('dir')) {
    ...
    if (name.indexOf('/') !== -1) {
      ...
      var self = this;
      this.driver.mkdir(this.path + '/' + name + '/', function (err) {
        if (!err) { self.touch(); }
        cb(err);
      });
    }
...
File.prototype.mkfile = function (name, cb) {
  if (this.driver.mkfile) {
    ...
    if (name.indexOf('/') !== -1) {
      ...
      var self = this;
      this.driver.mkfile(this.path + '/' + name, function (err) {
        if (!err) { self.touch(); }
        cb(err);
      });
    }
