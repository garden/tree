...
function file (vpath, cb) {
  ...
  driver.loadMeta(that.path, function (err, meta) {
    if (err && err.code !== 'ENOENT' && err.code !== 'ENOTDIR') {
      console.error('error', err.code, 'with metadata of', that.path);
    ...
    guessType(that, function (err, td) {  // td: type descriptor.
      if (err) {
      ...
      if (that.meta['Last-Modified'] === undefined) {
        that.touch();
        that.writeMeta();
      }
      cb(null, that);
...
function guessType(file, cb) {
  ...
  nodefs.stat(driver.relative(npath), function (err, stats) {
    ...
    if (stats.isDirectory()) {
      cb(null, type.fromName['dir']);
    } else if (stats.isFile()) {
      // Pick the type from metadata.
      cb(null, type.fromName[lookup(file, {})(
        'types[' + JSON.stringify(nodepath.extname(npath).slice(1)) + ']'
      )] || type.fromName[file.meta.type] || type.fromName['binary']);
    } else {
...
// The following three functions are directory-only.
...
File.prototype.mkfile = function (name, cb) {
  if (this.driver.mkfile) {
    // We don't want name to have slashes (avoid attacks).
    if (name.indexOf('/') !== -1) {
      console.error("fs:file:mkfile: cannot make file %s which has a /.", name);
    } else {
      var self = this;
